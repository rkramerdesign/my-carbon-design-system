...
var run = async (input = DEFAULT_PARAMETERS, storyId) => {
  var _a;
  let axe = ((_a = await import("./axe-7PYFLBUS.js")) == null ? void 0 : _a.default) || globalThis.axe, { config = {}, options = {} } = input;
  if (input.element) throw new re();
  let context = { include: document2 == null ? void 0 : document2.body, exclude: [".sb-wrapper", "#storybook-docs"] };
  if (input.context) {
    ...
    if (result) {
      ...
      if (reporting.addReport({ type: "a11y", version: 1, result, status: hasViolations ? getMode() : "passed" }), getIsVitestStandaloneRun() && hasViolations && getMode() === "failed") {
        if (!vitestMatchersExtended) {
          let { toHaveNoViolations } = await import("./matchers-7Z3WT2CE-XTDNQKKG.js");
          DTe.extend({ toHaveNoViolations }), vitestMatchersExtended = true;
        }
        DTe(result).toHaveNoViolations();
      }
    }
  } catch (e) {
